...
function WorkspaceCard({ workspace }) {
  ...
  const handleControl = (action) => {
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        action === "start"
          ? toast.success("Workspace is now running!")
          : toast.success("Workspace has stopped!");
        router.refresh();
      }
    });
  };
  ...
  return (
    <Card className="rounded-md bg-muted border-border/50">
      <CardHeader>
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="text-sm font-mono text-muted-foreground">
            $ Base technologies:{" "}
            <span className="font-medium">{workspace.technologies}</span>
          </div>
          ...
          <div className="space-y-1">
            {container.status === "running" && container.urls ? (
              <div className="font-mono">
                <p className="inline text-sm text-muted-foreground mr-1">
                  $ Access App:
                </p>
                <Link
              ...
              <div className="font-mono">
                <p className="inline text-sm text-muted-foreground mr-1">
                  $ Start workspace to access the app
                </p>
              </div>
            )}
          </div>
              ...
              <>
                ...
                >
                  {isPending ? (
                    ...
                    <Trash color="var(--background)" className="w-4 h-4" />
                  )}
                  Delete
                </Button>
              </>
